  ...
  return (
    <div id="first-view" className="first-view" style={{backgroundImage: `url('${movies && movies.backdrop_path ? `https://image.tmdb.org/t/p/original${movies.backdrop_path}` : noImg}')`}}>
      <div className="first-view__container">
        ...
        <div className="group">
          <div className="btn">
            <Link to="/" className="btn__play" onClick={() => setShowDetail(true)}>
            ...
            <Link to={`/movie/${movies && movies.id}`} className="btn__more">
      ...
      <MovieDetail selectedMovie={movies && movies} showPopup={showDetail} setShowPopup={setShowDetail} />
    </div>
  )
}
