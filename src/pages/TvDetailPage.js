...
const TvDetailPage = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      let url = `tv/${id}?api_key=${API_KEY}&language=en-US`;
    ...
    <div id="movie-detail" className="movie-detail">
      ...
      <div className="movie-detail__container">
        <div className="poster">
          <img src={`${movies && movies.poster_path ? `https://image.tmdb.org/t/p/original/${movies.poster_path}` : noImg}`} alt={movies && movies.original_name ? movies.original_name : 'Unknown'}/>
        </div>
        <div className="info">
          <h2 className="name">{movies && movies.original_name ? movies.original_name : 'Unknown'}</h2>
          <p className="match">
            <span className="match__vote">{movies && movies.vote_average.toString().replace('.', '')}% Match</span>
            <span className="match__year">{movies && movies.first_air_date.slice(0, 4)}</span>
