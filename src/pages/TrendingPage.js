...
const TrendingPage = () => {
  ...
  useEffect(() => {
    const getMovies = async () => {
      setLoading(false);
      let url = `trending/all/day?api_key=${API_KEY}&language=en-US&page=${currentPage}`;
  ...
  useEffect(() => {
    const getFilterMovies = () => {
      ...
      filterMovies = movies && movies.results.filter(movie =>
        parseInt(movie.release_date ? movie.release_date.slice(0, 4) : movie.first_air_date.slice(0, 4)) >= yearRange.min &&
        parseInt(movie.release_date ? movie.release_date.slice(0, 4) : movie.first_air_date.slice(0, 4)) <= yearRange.max &&
