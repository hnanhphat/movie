...
const TvShowPage = () => {
  ...
  useEffect(() => {
    const getMovies = async () => {
      setLoading(false);
      let url = `tv/popular?api_key=${API_KEY}&language=en-US&page=${currentPage}`;
  ...
  useEffect(() => {
    const getFilterMovies = () => {
      ...
      filterMovies = movies && movies.results.filter(movie =>
        parseInt(movie.first_air_date.slice(0, 4)) >= yearRange.min &&
        parseInt(movie.first_air_date.slice(0, 4)) <= yearRange.max &&
